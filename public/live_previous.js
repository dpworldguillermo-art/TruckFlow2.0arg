...
function currentShift(now=new Date()){
  ...
  let number='';
  if(hour>=7 && hour<19){
    start.setHours(7,0,0,0); end.setHours(19,0,0,0); number='1';
  } else if(hour>=19){
    start.setHours(19,0,0,0); end.setDate(end.getDate()+1); end.setHours(7,0,0,0); number='2';
  } else {
    start.setDate(start.getDate()-1); start.setHours(19,0,0,0); end.setHours(7,0,0,0); number='2';
  }
  return {start,end,number};
}

function previousShift(now=new Date()){
  const current=currentShift(now);
  const end=new Date(current.start);
  const start=new Date(end.getTime()-12*60*60*1000);
  const number=start.getHours()===7?'1':'2';
  const name=number==='1' ? 'TURNO ANTERIOR · TURNO 1 · 07:00 - 19:00' : 'TURNO ANTERIOR · TURNO 2 · 19:00 - 07:00';
  return {start,end,number,name};
}
...
function durationMinutes(row, referenceTime){
  const start=parseDateValue(row['Start Date']);
  if(!start) return null;
  let finish=referenceTime;
...
function avgMinutes(rows, referenceTime){
  const values=rows.map(r=>durationMinutes(r,referenceTime)).filter(Number.isFinite);
...
function renderLive(){
  const now=new Date();
  const shift=previousShift(now);
  const referenceTime=shift.end;
  const eligible=liveState.rows.filter(r=>{
    const st=normalizedStatus(r);
    return (st==='OK'||st==='COMPLETE') && inShift(r,shift);
  });
  const completed=eligible.filter(r=>normalizedStatus(r)==='COMPLETE');
  ...
  q('#activeNow').textContent=completed.length;
  q('#avgTotal').textContent=formatTime(avgMinutes(eligible,referenceTime));
  q('#avgActive').textContent=formatTime(avgMinutes(completed,referenceTime));

  renderTypeDetail(completed,referenceTime);
  renderDistribution(completed);
}

function renderTypeDetail(completed,referenceTime){
  ...
  if(!items.length){host.innerHTML='<div class="live-no-data">No hay operaciones completadas registradas para el turno anterior.</div>';return;}
  ...
  host.innerHTML=items.map(([type,rows],i)=>{
    ...
    const vals=rows.slice().sort((a,b)=>(parseDateValue(a['Start Date'])||0)-(parseDateValue(b['Start Date'])||0)).map(r=>durationMinutes(r,referenceTime));
    const avg=avgMinutes(rows,referenceTime);
    return `<div class="live-detail-row">
      <strong class="live-type">${escapeLive(type)}</strong>
      <div class="course-bar"><div class="course-fill" style="width:${pct}%;background:${color}"></div><span>Completadas</span><b>${rows.length}</b></div>
      <div class="type-average">${sparkline(vals,color)}<strong>${formatTime(avg)}</strong></div>
    </div>`;
  }).join('');
}

function renderDistribution(completed){
  ...
  if(!total){
    ...
    donut.innerHTML='<span>0<small>COMPLETADAS</small></span>';
    legend.innerHTML='<div class="live-no-data">Sin operaciones completadas para distribuir.</div>';
    return;
  }
  ...
  donut.innerHTML=`<span>${total}<small>COMPLETADAS</small></span>`;
